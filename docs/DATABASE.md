...
Проверка реального PostgreSQL выполнена в отдельной временной схеме на VPS: точное сохранение трёх направлений, компании с кавычкой, комментария, source_path и ссылки; повторная заявка не создала дубликат. Схема удалена после проверки. Это ещё не установка миграции в рабочую таблицу и не проверка Google-доставки.
