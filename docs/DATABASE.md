...
У клиентов и заявок уже независимые последовательности: `public.customers_id_seq` и `public.orders_id_seq`. Каждая начинается с 1, шаг — 1, зацикливание выключено. При создании нового клиента и его первой заявки разные счётчики могут выдать одинаковые числа. Независимость не означает обязательное различие чисел в разных таблицах.
...
В запросе и ответе API UUID называется `id`, в БД — `submission_id`: это одно значение. UUID не является паролем или разрешением на доступ. Сейчас именно он показывается в теме/теле письма и номере сообщения LOOP.

### Человекочитаемый номер

Для человека подходит **«Заявка №42»** или **`NS-42`**, где 42 — существующий `orders.id`. Отдельная колонка и счётчик для такого отображения не нужны. Префикс отличает номер заявки от номера клиента. UUID целесообразно сохранить как технический ключ API, повторов и связей.

Пример просмотра в DBeaver, только чтение:

```sql
SELECT id AS order_id,
       'NS-' || id::text AS request_number,
       customer_id,
       submission_id,
       created_at
FROM public.orders
...
Запрос вычисляет подпись, не меняя БД. Короткий номер в письмах/LOOP/API — предложенное дальнейшее изменение, **в текущем коде не включено**. Просто заменить UUID числом в форме нельзя: API проверяет формат, ключ используется в нескольких таблицах.
...
Миграции установлены предыдущими релизами. Эта редакция меняет только документацию: схема, данные и форматы ID не изменены, релиз на VPS не выпускался. Исторический `server/backfill_crm.py` переносил сохранившуюся SQLite-очередь в реестр без повторных писем; это не часть текущего приёма и не способ восстановить уже удалённые файлы.
