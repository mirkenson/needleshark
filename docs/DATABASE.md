...
Расширение для каталога от 10 сентября: nullable `product_slug`, `product_name`, `product_size`, `inquiry_type`, `quantity`, `source_path`. `quantity` — целое 1–1000000, остальные поля — текст; тип обращения проверяется сервером (`direct`, `sizing`, `wholesale`). Старые строки остаются с NULL. Миграция: `server/migrations/20260910_catalog_context.sql`. Содержимое `description` остаётся комментарием посетителя; в Google Sheets реквизиты товара и обращения добавляются в текст колонки «Задача». Эти сведения описывают заявку и не подтверждают наличие или стоимость товара.
